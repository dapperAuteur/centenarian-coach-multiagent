...
| [`script.md`](./script.md) | The **scripting blueprint**: the reusable per-lesson script template, the per-lesson runtime budget, and the per-lesson "must-cover" checklist. **Verbatim narration is written last**, module by module, after each module's lesson prose is final — so the words match the shipped course (see below). |

## Why the script is written last

Verbatim narration follows the finalized lesson prose; it does not lead it. If we
script before the lessons are written, the script drifts from the course and gets
re-recorded. So `script.md` is a **blueprint now** (template + budget + coverage
checklist, all derived from the approved lesson plan) and the **verbatim narration
for each module is filled in only after that module's lessons are authored and
approved**. The production guide, by contrast, depends on none of the lesson
...
- **Script-writing trigger (per module):** once a module's lessons are authored
  and approved, write that module's verbatim narration into `script.md`. Module 0
  scripts can be written as soon as Module 0's lessons land; Modules 1–6 scripts
  follow their lessons after the tone/citation checkpoint. The runtime budget and
  structure are fixed up front so only the words are added.
...
in [`../lessons/`](../lessons/). The video inherits the lesson's APA-7 references —
it does not invent new ones on camera.
